...
void spinFlop_MMC_pin(vector<vector<bool> >  J_pos, vector<vector<bool> >  J_val, vector<bool>& s1, vector<bool>& s2, double betha, int chosenSpin, MTRand& MTrnd)
{
	int N = s1.size();
	double dE = 0;
	double expBdE;
...
vector<vector<int> > getCompareMatrix_pin(vector<bool> spin1, vector<bool> spin2, vector<bool> spinDiff, int sideX, int sideY, int chosenSpin)
{
	vector<int> vecGray(2*sideX+2,127);
	vector<vector<int> > Comp(2*sideY+2,vecGray);
	int N = spin1.size();
	int chSpinVal = (spin2[chosenSpin] == true) ? 191 : 63;
...
int main()
{
	...
	int N;
	string fileP = "06_SG_Param_MMC_vsPin.dat";
	...
	int qABSampling = loadParameter_int(fileP,45); 
	double bin_qAB = loadParameter_double(fileP,48);
	int spinX = loadParameter_int(fileP,51);
	int spinY = loadParameter_int(fileP,54);
	int spinPixelSize = loadParameter_int(fileP,57);
	
	chosenSpinIndex = "_" + chosenSpinIndex;
	...
	vector<double> vecZeros_D(2,0.0);
	...
	cout << "Kivalasztott spin indexe: " << chosenSpin+1 << endl;
	cout << endl;
	...
	for (double t=Tmin; t<=Tmax; t+=dT)
	{
		...
		spin2[chosenSpin] = chosenSpinVal;				//Kivalasztott spin beallitesa iranyra
		spinCompareM = getCompareMatrix_pin(spin1,spin2,spinDiff,spinX,spinY,chosenSpin);
...
///-------------ido fejlesztes----------------///
		...
		for (int time=0; time<timeMax; time++)	
		{
			...
			spinFlop_MMC_pin(J_pos,J_val,spin1,spin2,betha,chosenSpin,MTrnd);
			...
			if (time % eSampling == 0)
			{
				...
				E2[1][int(time/eSampling)] = getEnergy_justPairing(J_pos,J_val,spin2);
			}
			if (time % mSampling == 0)
			{
		...
		saveMatDoub(E2,"E" + strN + strT + chosenSpinIndex,"Energy values, sampling freq="+convert_ItoS(eSampling));
		...
		saveMatDoub(q_EA2,"q_EA" + strN + strT + chosenSpinIndex,"q_EA, Edward-Anderson order-parameter, sampling freq="+convert_ItoS(qEASampling));
		saveVecDoub(s_avg2,"s_avg" + strN + strT + chosenSpinIndex,"<si> vector");
		saveVecDoub(effField2,"effField" + strN + strT + chosenSpinIndex,"");
		
		infos.push_back(""); infos.push_back("");
		infos.push_back("Alternativ rendszer:"); infos.push_back(convert_ItoS(chosenSpin+1) + ". spin megfogva " + chosenSpinVal_ + " iranyba");
		...
		saveMatDoub(C,"C" + strN + strT + chosenSpinIndex,"Connected-Correlation C Matrix");
		saveMatDoub3d(C,"C" + strN + strT + "_3D" + chosenSpinIndex,"Connected-Correlation Matrix for 3D plot");
		saveVecDoub(C_CDF,"C" + strN + strT + "_CDF" + chosenSpinIndex,"C Matrix's UpperTriangular for CDF");
		savePDDF_vec(C_CDF,bin_C,-1,1,"C" + strN + strT + "_PDDF" + chosenSpinIndex,"C matrix Vsz.-suruseg eo.-fv.");
		savePDDF_vec_alter(C_CDF,bin_C,-1,1,"C" + strN + strT + "_PDDF_alter" + chosenSpinIndex,"C matrix Vsz.-suruseg eo.-fv.");
		...
		saveMatDoub(C,"K" + strN + strT + chosenSpinIndex,"Correlation K Matrix");
		saveMatDoub3d(C,"K" + strN + strT + "_3D" + chosenSpinIndex,"Correlation Matrix for 3D plot");
		saveVecDoub(C_CDF,"K" + strN + strT + "_CDF" + chosenSpinIndex,"Correlation Matrix's UpperTriangular for CDF");
		savePDDF_vec(C_CDF,bin_C,-1,1,"K" + strN + strT + "_PDDF" + chosenSpinIndex,"Correlation matrix Vsz.-suruseg eo.-fv.");
		savePDDF_vec_alter(C_CDF,bin_C,-1,1,"K" + strN + strT + "_PDDF_alter" + chosenSpinIndex,"Correlation matrix Vsz.-suruseg eo.-fv.");
	...
	saveMatInt(J,"J" + strN + chosenSpinIndex,"J_Matrix, Frustration Rate: " + convert_DtoS(fi_J));
